...
<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$diretorio = '../img/clientes_vips/';
			if(!is_dir($diretorio)){
			   mkdir($diretorio);
			}

		if($img["name"] != "") {
			  $nome="cliente".date("dmYhis")."."; 
		...
		$array = array (
			'titulo_cliente_vip' => $titulo,
			'img_cliente_vip' => $NomeImg,
			'url_cliente_vip' => $url
		);

		$grava = DBCreate('clientes_vips', $array);

		if ($grava) {
			echo "<script> alert('CLIENTE CADASTRADO COM SUCESSO!') </script>";
			echo "<script>location.href=('sis_cliente.php')</script>";
		} else { 
...
<form action="<?php echo htmlspecialchars($_SERVER[$PHP_SELF]); ?>" method="post" enctype="multipart/form-data" accept-charset="utf-8">
	<table width="80%" border="0" cellspacing="3" cellpadding="0" class="sistable">
		<tbody>
			...
			<tr>
				<td>
				<label for="url">Link Cliente:</label>
				<br />
